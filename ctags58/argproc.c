 ...
 */
{
	...
	ap = argv [argc-1];
	if (0 == strcmp ("&", ap))
		IOSEXIT(background_process (--argc, argv));
	if ('&' == ap [strlen (ap)-1])
		{
		ap [strlen (ap)-1] = '\0';
		IOSEXIT(background_process (argc, argv));
	...
	*ac = item_count;
	...
	if ((in != NULL) && (NULL == freopen (in, "r", stdin, "mbc=32", "mbf=2")))
		...
		IOSEXIT(EXIT_ERR);				/* Is a fatal error				*/
		}
	if ((out != NULL) && (NULL == freopen (out, outmode, CTAGSSTDOUT, "mbc=32", "mbf=2")))
		{
		perror (ap);			/* Error, can't write or append	*/
		IOSEXIT(EXIT_ERR);				/* Is a fatal error				*/
		}
#ifdef DEBUG
	fprintf (CTAGSSTDERR, "Arglist:\n");
	for (j = 0; j < *ac;  ++j)
		fprintf (CTAGSSTDERR, "argv[%d] = '%s'\n", j, argv [j]);
#endif
	return 0;
}
...
short iosb [4];
	...
	if (0 == child_st [0])
		{
#ifdef DEBUG
		fprintf (CTAGSSTDERR, "Waiting for Child Process to Finnish . . .\n");
#endif
...
{
#ifdef DEBUG
	fprintf (CTAGSSTDERR, "Child Completion AST\n");
#endif
	if (child_st [0] == 0)
		child_st [0] = 1;
}
...
#ifdef DEBUG
	fprintf (CTAGSSTDERR, "Pipe Mailbox Name = '%s'\n", mbxname);
#endif
...
#ifdef DEBUG
	fprintf (CTAGSSTDERR, "Subprocess's Pid = %08X\n", pid);
#endif
...
#ifdef DEBUG
	fprintf (CTAGSSTDERR, "%s\n", command);
#endif
	fprintf (CTAGSSTDERR, "%08X\n", pid);
	return (EXIT_OK);
}
