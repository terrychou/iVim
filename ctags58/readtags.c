					   ...
					   const char *const name, const int options)
{
	...
	{
#ifdef DEBUG
		CTAGSPRINTF ("<performing binary search>\n");
#endif
	...
	{
#ifdef DEBUG
		CTAGSPRINTF ("<performing sequential search>\n");
#endif
...
#define sep (first ? (first = 0, separator) : empty)
	...
	if (extensionFields)
	{
		if (entry->kind != NULL  &&  entry->kind [0] != '\0')
			CTAGSPRINTF ("%s\tkind:%s", sep, entry->kind);
		if (entry->fileScope)
			CTAGSPRINTF ("%s\tfile:", sep);
#if 0
		if (entry->address.lineNumber > 0)
			CTAGSPRINTF ("%s\tline:%lu", sep, entry->address.lineNumber);
#endif
		for (i = 0  ;  i < entry->fields.count  ;  ++i)
			CTAGSPRINTF ("%s\t%s:%s", sep, entry->fields.list [i].key,
...
static void findTag (const char *const name, const int options)
{
	...
	if (file == NULL)
	{
		fprintf (CTAGSSTDERR, "%s: cannot open tag file: %s: %s\n",
...
extern int main (int argc, char **argv)
{
	...
	if (argc == 1)
	{
		fprintf (CTAGSSTDERR, Usage, ProgramName);
		IOSEXIT(1);
	}
	for (i = 1  ;  i < argc  ;  ++i)
	{
			...
			for (j = 1  ;  arg [j] != '\0'  ;  ++j)
			{
				switch (arg [j])
				{
					...
					case 's':
						...
						break;
					default:
						fprintf (CTAGSSTDERR, "%s: unknown option: %c\n",
									ProgramName, arg[j]);
						IOSEXIT(1);
						break;
				}
