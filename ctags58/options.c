...
{
	CTAGSPRINTF (INVOCATION, getExecutableName ());
}
...
static void printOptionDescriptions (const optionDescription *const optDesc)
{
	...
	for (i = 0 ; optDesc [i].description != NULL ; ++i)
	{
		if (! Option.etags || optDesc [i].usedByEtags)
			CTAGSPUTS (optDesc [i].description);
	}
}
...
static void printFeatureList (void)
{
	...
	{
		if (i == 0)
			CTAGSPRINTF ("  Optional compiled features: ");
		CTAGSPRINTF ("%s+%s", (i>0 ? ", " : ""), Features [i]);
#ifdef CUSTOM_CONFIGURATION_FILE
		if (strcmp (Features [i], "custom-conf") == 0)
			CTAGSPRINTF ("=%s", CUSTOM_CONFIGURATION_FILE);
#endif
	}
	if (i > 0)
		CTAGSPUTCHAR ('\n');
...
static void printProgramIdentification (void)
{
	...
	CTAGSPRINTF ("  Compiled: %s, %s\n", __DATE__, __TIME__);
	CTAGSPRINTF ("  Addresses: <%s>, %s\n", AUTHOR_EMAIL, PROGRAM_URL);
	printFeatureList ();
}
...
static void installHeaderListDefaults (void)
{
	...
	if (Option.verbose)
	{
		CTAGSPRINTF ("    Setting default header extensions: ");
...
extern void previewFirstOption (cookedArgs* const args)
{
	while (cArgIsOption (args))
	{
		...
		{
			fprintf (CTAGSSTDERR, "No options will be read from files or environment\n");
...
#ifdef IOS_FUNCTION
static void ctags_restore_option(void);
#endif
...
static void ctags_restore_option(void) {
    Option.include.fileNames = FALSE;
    Option.include.qualifiedTags = FALSE;
    Option.include.fileScope = TRUE;
    
	Option.extensionFields.access = FALSE;
	Option.extensionFields.fileScope = TRUE;
	Option.extensionFields.implementation = FALSE;
	Option.extensionFields.inheritance = FALSE;
	Option.extensionFields.kind = TRUE;
	Option.extensionFields.kindKey = FALSE;
	Option.extensionFields.kindLong = FALSE;
	Option.extensionFields.language = FALSE;
	Option.extensionFields.lineNumber = FALSE;
	Option.extensionFields.scope = TRUE;
	Option.extensionFields.signature = FALSE;
	Option.extensionFields.typeRef = TRUE;
