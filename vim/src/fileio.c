...
#endif
#ifdef FEAT_GUI_IOS
    mch_ios_post_buffer_write(buf, fname);
#endif
    return retval;
}
    ...
    int		orig_mode;
{
	...
	if (saved == OK)
	{
		...
		if (savebuf != NULL && buf_validNEW(savebuf) && buf == curbuf)
		{
	...
	if (savebuf != NULL && buf_validNEW(savebuf))
    ...
    exarg_T	*eap;
{
	...
	if (buf->b_ml.ml_mfp != NULL)
	{
	    ...
	    if (retval == FAIL || !buf_validNEW(buf))
		break;
	}
    }
