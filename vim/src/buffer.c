...
# endif
	 ...
	 */
	if (buf_validNEW(old_curbuf) && old_curbuf->b_ml.ml_mfp != NULL)
	{
	    aco_save_T	aco;
    ...
    int		flags;
{
    ...
    if (!buf_validNEW(buf))	    /* autocommands may delete the buffer */
	return;
...
# endif
	...
	if (!buf_validNEW(old_curbuf) || old_curbuf == curbuf)
    ...
    int action;
{
     ...
     */
    if (buf != curbuf && buf_validNEW(buf) && buf->b_nwindows == 0)
...
#ifdef FEAT_AUTOCMD
	if (au_new_curbuf != NULL && buf_validNEW(au_new_curbuf))
	    buf = au_new_curbuf;
# ifdef FEAT_JUMPLIST
	else
# endif
...
# ifdef FEAT_EVAL
    if (buf_validNEW(prevbuf) && !aborting())
# else
    if (buf_validNEW(prevbuf))
# endif
#endif
    {
#ifdef FEAT_SYN_HL
    ...
    if ((buf_validNEW(buf) && buf != curbuf
# ifdef FEAT_EVAL
	    && !aborting()
# endif
# ifdef FEAT_WINDOWS
    ...
    int keep_tabs;		/* keep current tabs, for ":tab drop file" */
{
	...
	for (wp = firstwin; wp != NULL; wp = wpnext)
	{
							...
							|| !bufIsChanged(buf))
		{
			...
			if (!win_valid(wp) || !buf_validNEW(buf))
			{
...
#ifdef FEAT_AUTOCMD
	    if (!buf_validNEW(buf))	/* autocommands deleted the buffer!!! */
	    {
#if defined(HAS_SWAP_EXISTS_ACTION)
		swap_exists_action = SEA_NONE;
# endif
